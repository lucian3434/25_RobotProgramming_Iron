  ...
  yahboomcar_msgs__msg__ImageMsg__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yahboomcar_msgs__msg__ImageMsg * data =
      (yahboomcar_msgs__msg__ImageMsg *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yahboomcar_msgs__msg__ImageMsg__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          yahboomcar_msgs__msg__ImageMsg__fini(&data[i]);
        }
        free(data);
        return false;
      }
