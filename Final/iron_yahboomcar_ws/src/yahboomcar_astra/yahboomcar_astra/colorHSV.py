...
from yahboomcar_msgs.msg import Position, ColorData
#common lib
import os
import threading
import math
...
class Color_Identify(Node):
    def __init__(self,name):
        ...
        self.pub_colorData = self.create_publisher(ColorData, '/color_data', 10)
        self.index = 2
        self.Roi_init = ()
        self.hsv_range = ()
        self.circle = (0, 0, 0)
    ...
    def process(self, rgb_img, action):
        ...
        # added to publish HSV value of center pixel
        hsv_img = cv.cvtColor(rgb_img, cv.COLOR_BGR2HSV)
        colorPixel = ColorData()
        colorPixel.hue = int(hsv_img[240, 320, 0])
        colorPixel.saturation = int(hsv_img[240, 320, 1])
        colorPixel.value = int(hsv_img[240, 320, 2])
        self.pub_colorData.publish(colorPixel)
        
        hsvText = f'HSV: ({colorPixel.hue}, {colorPixel.saturation}, {colorPixel.value})'
        cv.putText(rgb_img, hsvText, (30, 60), cv.FONT_HERSHEY_SIMPLEX, 0.6, (100, 200, 200), 1)

        cv.line(rgb_img, (320, 220), (320, 260), (0, 0, 255), 2)
        cv.line(rgb_img, (300, 240), (340, 240), (0, 0, 255), 2)
